...
cd ../gcc/gcc-UBER && rm -rf * && git reset --hard && git fetch uu linaro-7.x && git checkout FETCH_HEAD && cd ../../;
...
export UBER_PATH=$DIR/out/linaro-arm-eabi-7.x;
...
then
    ...
    echo "${bldgrn} ${txtrst}"
    echo "${bldgrn}Your LINARO 7.x arm-eabi Toolchain has completed successfully!!! ${txtrst}"
    ...
    echo "${bldred}**************ERROR***************** ${txtrst}"
    echo " "
    echo "${bldred}Error Log is found at:${txtrst}${red} $DIR/out/LINARO-7.x.log ${txtrst}"
    echo " "
